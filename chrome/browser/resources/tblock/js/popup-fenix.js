...
const statsStr = '{{count}} ({{percent}}%)';
const domainsHitStr = '{{count}} out of {{total}}';
...
// Update title for switch button instead i18n
...
const switchTitleContainer = document.getElementById('switch');

function addSwitchTitle() {
    const body = document.body;

    if (body.classList.contains('off')) {
        switchTitleContainer.setAttribute('title', 'Click to enable tomi adBlock for this site.');
    } else {
        switchTitleContainer.setAttribute('title', 'Click to disable tomi adBlock for this site.\nCtrl+click to disable tomi adBlock only on this page.');
    }
}
